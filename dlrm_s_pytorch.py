...
try:
    ...
    print("Warning: cafe_torchrec modules not found. CAFE flag will not work.")
    HotSketchManager = None
    CafeEmbeddingBagCollection = None
# === CAFE 模块导入结束 ===
...
class DLRM_Net(nn.Module):
    ...
    def create_emb(self, m, ln, weighted_pooling=None):
        ...
        for i in range(0, ln.size):
            ...
            # === CAFE 模块创建 (修改) ===
            # 检查 self.cafe_manager (在 __init__ 中设置)
            if self.cafe_manager is not None:
                if weighted_pooling is not None:
                     # 我们的 CafeEmbeddingBagCollection 不支持 per_sample_weights
                     sys.exit("ERROR: CAFE-TorchRec (cafe_torchrec_embedding.py) does not support weighted_pooling.")
                
                # 'm' (m_spa) 是 DLRM 的全局嵌入维度
                EE = CafeEmbeddingBagCollection(
                    embedding_dim=m, 
                    hot_feature_count=self.cafe_args.cafe_global_hot_feature_count,
                    hash_table_size=self.cafe_args.cafe_per_table_hash_table_size,
                    hotsketch_manager=self.cafe_manager, # 传入全局 manager
                    device=self.cafe_device, # 传入在 run() 中确定的 device
                    hot_table_name=f"cafe_hot_{i}", # EBC 内部的唯一表名
                    hash_table_name=f"cafe_hash_{i}" # EBC 内部的唯一表名
                )
    ...
    def __init__(
        ...
        # === CAFE 成员变量 (新增) ===
        self.cafe_manager = cafe_manager
        self.cafe_device = cafe_device
        self.cafe_args = cafe_args
        # === CAFE 成员变量结束 ===
...
def run():
    ...
    # === CAFE-TorchRec 参数 (新增) ===
    parser.add_argument("--cafe-torchrec-flag", action="store_true", default=False,
                        help="启用 CAFE-TorchRec 嵌入模块")
    parser.add_argument("--cafe-cpp-lib-path", type=str, default="cafe_torchrec/cpp/libhotsketch.so",
                        help="编译好的 C++ HotSketch 库 (.so) 的路径")
    parser.add_argument("--cafe-global-hot-feature-count", type=int, default=100000,
                        help="HotSketch 管理器跟踪的全局热特征总数 (n)")
    parser.add_argument("--cafe-per-table-hot-feature-count", type=int, default=10000,
                        help="每个 CAFE 模块内部 EBC 热表的大小")
    parser.add_argument("--cafe-per-table-hash-table-size", type=int, default=100000,
                        help="每个 CAFE 模块内部 EBC 哈希表的大小")
    parser.add_argument("--cafe-sketch-threshold", type=float, default=500.0,
                        help="HotSketch 区分冷热特征的初始分数阈值")
    parser.add_argument("--cafe-sketch-adjust", type=int, default=1,
                        help="允许 HotSketch 动态调整阈值 (1=启用, 0=禁用, 默认=1)")
    parser.add_argument("--cafe-sketch-alpha", type=float, default=1.0000005,
                        help="HotSketch 分数衰减因子 (alpha)")
    # === CAFE-TorchRec 参数结束 ===
    ...
    # === CAFE C++ 编译和 Manager 初始化 (新增) ===
    global global_hotsketch_manager
    global_hotsketch_manager = None
    if args.cafe_torchrec_flag:
        if HotSketchManager is None:
            sys.exit("ERROR: --cafe-torchrec-flag is set, but cafe_torchrec modules could not be imported.")
        
        # 1. 编译 C++
        lib_path = args.cafe_cpp_lib_path
        # 假设 sketch.cpp 在 .so 路径的父目录下的 'cpp' 目录中，例如 cafe_torchrec/cpp/sketch.cpp
        cpp_file_path = os.path.join(os.path.dirname(lib_path), 'sketch.cpp')

        if not os.path.exists(lib_path) and os.path.exists(cpp_file_path):
            print(f"Compiling HotSketch C++ library: {cpp_file_path} -> {lib_path}")
            # 编译命令基于 test_cafe_full_step.py
            compile_cmd = (
                f"g++ -fPIC -shared -o {lib_path} -g -rdynamic -mavx2 -mbmi "
                f"-mavx512bw -mavx512dq --std=c++17 -O3 -fopenmp {cpp_file_path}"
            )
            ret = os.system(compile_cmd)
            if ret != 0:
                sys.exit(f"ERROR: C++ HotSketch library compilation failed!")
            print("C++ compile successful.")
        elif not os.path.exists(lib_path):
             sys.exit(f"ERROR: HotSketch library not found at {lib_path} and source {cpp_file_path} not found.")

        # 2. 初始化 HotSketchManager (全局单例)
        print("Initializing HotSketchManager...")
        try:
            # 将命令行传入的 int (1或0) 转换为 bool
            adjust_bool = bool(args.cafe_sketch_adjust)
            
            global_hotsketch_manager = HotSketchManager(
                library_path=lib_path,
                hot_feature_count=args.cafe_global_hot_feature_count,
                initial_threshold=args.cafe_sketch_threshold,
                adjust_threshold=adjust_bool, # hotsketch_manager.py 期望一个 bool
                alpha=args.cafe_sketch_alpha
            )
            print(f"HotSketchManager initialized (adjust_threshold={adjust_bool}).")
        except Exception as e:
            # 打印更详细的错误
            import traceback
            traceback.print_exc()
            sys.exit(f"ERROR: Failed to initialize HotSketchManager: {e}")
    # === CAFE 初始化结束 ===
    ...
    dlrm = DLRM_Net(
        ...
        # === CAFE 实例化参数 (新增) ===
        cafe_manager=global_hotsketch_manager,
        cafe_device=device,
        cafe_args=args
        # === CAFE 实例化参数结束 ===
    )
            ...
            while k < args.nepochs:
                ...
                for j, inputBatch in enumerate(train_ld):
                    ...
                    with record_function("DLRM backward"):
                        ...
                        # === CAFE: Update HotSketch and Migrate (新增) ===
                        if args.cafe_torchrec_flag:
                            # 此调用必须在 E.backward() 之后 (以读取 .grad)
                            # 且在 optimizer.step() 之前 (以便迁移的权重能被优化器更新)
                            with record_function("CAFE update and migrate"):
                                # dlrm.emb_l 现在包含 CafeEmbeddingBagCollection 实例
                                for emb_module in dlrm.emb_l:
                                    # 检查模块是否是我们的 CAFE 模块 (以防万一)
                                    if hasattr(emb_module, 'update_and_migrate'):
                                        emb_module.update_and_migrate()
                        # === CAFE 更新结束 ===
