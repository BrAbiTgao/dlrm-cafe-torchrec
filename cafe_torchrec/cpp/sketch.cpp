...
extern "C" {
    ...
    int ins[MAX_BUFFER], que[MAX_BUFFER];
    ...
    public:
        ...
        SS(float Threshold = 200, int lim = 130670, int adjust_thres=1): 
            ...
            if (addr == NULL) return;
            // === 修改开始 ===
            // 强制 n1 和 n2 至少为 1，防止 lim 过小时导致 % 0 错误
            n1 = std::max(1, (int)(lim * 0.9));
            n2 = std::max(1, (int)(lim * 0.1));
            // === 修改结束 ===
            b = new Bucket[n1];
            b2 = new Bucket2[n2];
            for (int i = 1; i < lim; ++i)
        ...
        void reset() {
            printf("reset executed: real_n=%d, lim=%d\n", real_n, lim); // 添加打印以便调试
            fflush(stdout);
            ...
            // === 关键安全检查 ===
            if (l < lim) {
                printf("reset safe exit: found %d features < capacity %d. Setting threshold to min or 0.\n", l, lim);
                if (l > 0) {
                    Threshold = vec[0].first;
                } else {
                    Threshold = 0;
                }
                real_n = l;
    ...
    void init(int n, float Threshold, int adjust_thres, double alp){
        ss = new SS(Threshold, n, adjust_thres);
        cout << "alp: " << alp << endl;
        alpha = alp;
    }
